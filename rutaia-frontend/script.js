...
const normalizeUser = (u)=> ({id:u.id, nombre:u.nombre, email:u.email, role:ROLES[u.rol]});
...
function completeAuth(res, role){
  const user = normalizeUser(res.usuario);
  if(user.role!==role){
...
function bindLogin({formId, role, fieldId, idInput, passInput}){
  ...
  form.addEventListener("submit", async (e)=>{
    ...
    if(!identificador || !password){ setFieldError(fieldId, "Completa todos los campos."); return; }

    const btn = form.querySelector("button[type=submit]");
    btn.disabled = true; btn.textContent = "Ingresando…";
    try{
      completeAuth(await api.login(identificador, password), role);
    }catch(err){
...
function bindRegister({formId, role, fields, read, call, btnText}){
  ...
  form.addEventListener("submit", async (e)=>{
    ...
    try{
      completeAuth(await call(data), role);  
      form.reset();
    }catch(err){
...
function checkCommon(f, i, data, pass2){
  let ok = true;
  if(!data.nombre){ setFieldError(f.nombre); ok=false; }
  if(!EMAIL_RE.test(data.email)){ setFieldError(f.email, "Escribe un correo válido."); ok=false; }
  if(data.password.length<6){ setFieldError(f.pass); ok=false; }
  else if(pass2!==undefined && $(pass2).value!==data.password){ setFieldError(f.pass2); ok=false; }
  return ok;
}
...
async function loadCatalogos(){
  ...
  fillSelect($("courseNivel"), state.niveles);    // curso.nivel_id
}
